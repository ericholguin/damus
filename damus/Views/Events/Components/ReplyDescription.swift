...
struct ReplyDescription: View {
    ...
    var body: some View {
        if event.known_kind == .highlight {
            (Text(Image(systemName: "highlighter")) + Text(verbatim: "\(highlight_desc(ndb: ndb, event: event, replying_to: replying_to))"))
                .font(.footnote)
...
func highlight_desc(ndb: Ndb, event: NostrEvent, replying_to: NostrEvent?, locale: Locale = Locale.current) -> String {
    let desc = make_reply_description(event, replying_to: replying_to)
    let pubkeys = desc.pubkeys
    let n = desc.others
    ...
    if uniqueNames.count > 1 {
        let othersCount = n - pubkeys.count
        if othersCount <= 0 {
            return String(format: NSLocalizedString("Highlighted %@ & %@", bundle: bundle, comment: "Label to indicate that the user highlighted 2 users."), locale: locale, uniqueNames[0], uniqueNames[1])
        } else {
            return String(format: localizedStringFormat(key: "replying_to_two_and_others", locale: locale), locale: locale, othersCount, uniqueNames[0], uniqueNames[1])
        }
    }

    return String(format: NSLocalizedString("Highlighted %@", bundle: bundle, comment: "Label to indicate that the user highlighted 1 user."), locale: locale, uniqueNames[0])
}
